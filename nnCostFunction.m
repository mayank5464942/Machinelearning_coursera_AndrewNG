...
q= size(Y,2);
J1 = -sum(Y.*log(a3)+(ones(m,q)-Y).*log(ones(m,q)-a3))/m ;
J =sum(J1);
...
Theta1_grad=(del1+lambda*[zeros(size(Theta1,1),1) Theta1(:,2:end)])/m;
Theta2_grad=(del2+lambda*[zeros(size(Theta2,1),1) Theta2(:,2:end)])/m;
